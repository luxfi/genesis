...
func main() {
	...
	fmt.Printf("Writing chain config to BadgerDB at: %s\n", dbPath)
	...
	// Create a proper chain config for Lux mainnet
	config := &params.ChainConfig{
		...
		ShanghaiTime:        func() *uint64 { t := uint64(0); return &t }(),
		CancunTime:          func() *uint64 { t := uint64(0); return &t }(),
		PragueTime:          nil,
		VerkleTime:          nil,
	...
	fmt.Printf("\n✅ Successfully wrote chain configuration!\n")
	fmt.Printf("Chain ID: 96369\n")
	fmt.Printf("All forks enabled from genesis\n")
	fmt.Printf("Cancun fork included\n")
}