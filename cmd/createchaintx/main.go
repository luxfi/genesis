...
// /v1/chain/P with method="platform.getBlockchains" and parses the
...
func platformGetBlockchains(ctx context.Context, uri string) ([]platformBlockchain, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, strings.TrimRight(uri, "/")+"/v1/chain/P", body)
	if err != nil {
		return nil, err
	}
