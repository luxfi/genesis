...
// NewLaunchBFTSimpleCmd creates a simplified launch-bft command
func NewLaunchBFTSimpleCmd(app *application.Genesis) *cobra.Command {
	var (
		dataDir     string
		httpPort    int
		stakingPort int
	...
	cmd := &cobra.Command{
		Use:   "launch-bft-simple",
		Short: "Launch Lux node with simplified BFT consensus (k=1)",
		Long:  `Launch a Lux node with Byzantine Fault Tolerant consensus using a single validator with simplified genesis.`,
