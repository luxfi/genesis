...
func main() {
	fmt.Println("=== Full Migration Tool ===")
	...
	// Create directories
	os.MkdirAll(filepath.Dir(ethdbPath), 0755)
	os.MkdirAll(filepath.Dir(vmdbPath), 0755)
	
	// Open source
	source, err := pebble.Open(sourceDB, &pebble.Options{ReadOnly: true})
	if err != nil {
		log.Fatal("Failed to open source:", err)
	}
	defer source.Close()
	
	// Open destination fresh
	fmt.Println("Opening destination database...")
	ethdb, err := badgerdb.New(ethdbPath, nil, "", prometheus.DefaultRegisterer)
	if err != nil {
		log.Fatal("Failed to open destination:", err)
	}
	defer ethdb.Close()
	...
	for iter.First(); iter.Valid(); iter.Next() {
		...
		} else if keyLen == 42 && actualKey[0] == 'h' && actualKey[9] == 'n' {
			// Canonical mapping
			canonical++
			num := binary.BigEndian.Uint64(actualKey[1:9])
		...
		if time.Since(lastReport) > 2*time.Second {
			...
			fmt.Printf("Progress: %d keys (%.0f/sec) - H:%d B:%d R:%d S:%d\n",
				count, rate, headers, bodies, receipts, state)
			lastReport = time.Now()
		}
	}
	...
	for i := uint64(0); i <= 1082780; i++ {
		...
		batch.Put(tdKey, td.Bytes())
		tdCount++
		
		if tdCount%10000 == 0 {
			batch.Write()
			batch.Reset()
			fmt.Printf("  TD progress: %d\n", i)
		}
	}
	batch.Write()
