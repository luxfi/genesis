...
// NewLaunchNodeCmd creates the launch-node command
func NewLaunchNodeCmd(app *application.Genesis) *cobra.Command {
	var (
		binaryPath    string
		dataDir       string
		networkID     uint32
	...
	cmd := &cobra.Command{
		Use:   "launch-node",
		Short: "Launch a Lux node with proper configuration",
		Long: `Launch a Lux node with the specified configuration.
This command provides a type-safe way to start luxd with proper settings.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("Admin API: http://localhost:%d/ext/admin\n", httpPort)
			fmt.Printf("Metrics: http://localhost:%d/ext/metrics\n", httpPort)
